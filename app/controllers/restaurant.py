...
@lru_cache
async def get_restaurant_by_id(id: str, db: Session):
    """
...
async def update_restaurant(id: str, data: dict, db: Session):
    """
...
async def delete_restaurant(id: str, db: Session):
    """
...
async def get_restaurants_statistics(db: Session, latitude: float, longitude: float, radius: int):
    ...
    return db.query(Restaurant).filter(
        Restaurant.lat > latitude - radius,
        Restaurant.lat < latitude + radius,
        Restaurant.lng > longitude - radius,
        Restaurant.lng < longitude + radius
    ).all()
